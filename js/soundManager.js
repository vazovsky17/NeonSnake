...
(function () {
    ...
    const sounds = {
        ...
        eat: () => {
            playTone(900, 'triangle', 0.08, 0.12);
            setTimeout(() => playTone(1300, 'sine', 0.06, 0.08), 70);
        },
        levelup: () => {
            playTone(1200, 'sine', 0.12, 0.15);
            setTimeout(() => playTone(1600, 'sine', 0.1, 0.12), 120);
        },
        gameover: () => {
            playTone(300, 'sawtooth', 0.24, 0.16);
            setTimeout(() => playTone(180, 'sine', 0.12, 0.12), 160);
        },
        start: () => {
            playTone(1000, 'sine', 0.09, 0.12);
            setTimeout(() => playTone(1400, 'sine', 0.07, 0.1), 80);
        },
        pause: () => playTone(600, 'sine', 0.06, 0.09),
        resume: () => playTone(900, 'sine', 0.06, 0.09),

        // ✅ НОВЫЙ ЗВУК: New Personal Record!
        newrecord: () => {
            // Восходящая трель — эффект "успеха"
            playTone(1000, 'sine', 0.08, 0.1);
            setTimeout(() => playTone(1300, 'sine', 0.08, 0.12), 80);
            setTimeout(() => playTone(1600, 'sine', 0.1, 0.14), 160);
            setTimeout(() => playTone(1800, 'triangle', 0.12, 0.1), 260);
        }
    };
    ...
    try {
        ...
        if (stored && typeof stored.volume === 'number') {
            window.soundManager.setVolume(stored.volume);
        }
    } catch (e) {
